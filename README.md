...
1. **安装 Python**（没有的话）：[python.org/downloads/windows/](https://www.python.org/downloads/windows/) 下载安装，**务必勾选「Add Python to PATH」**
2. **下载 Eva**：[最新版 ZIP](https://lnpiao.cn/eva/) → 解压
3. **启动**：
   - Windows：双击 `start_windows.bat`
   - Linux/macOS：双击 `start_linux.sh`
4. 按引导配置 API Key，开始使用
4. 粘贴 Key，开始使用
...
| 系统 | 文件 | 操作 |
|------|------|------|
| 🪟 Windows | `start_windows.bat` | 双击运行 |
| ⚡ Windows (PowerShell) | `start_windows.ps1` | 右键 → 使用 PowerShell 运行 |
| 🐧 Linux | `start_linux.sh` | 双击运行（或在终端执行）|
